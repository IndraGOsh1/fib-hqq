...
export interface CarpetaPersonal { username:string; anotaciones:Anotacion[]; documentos:{id:string;nombre:string;descripcion:string;fecha:string}[] }
...
export function getCarpeta(username:string): CarpetaPersonal {
  if (!CarpetasDB.has(username)) CarpetasDB.set(username, { username, anotaciones:[], documentos:[] })
