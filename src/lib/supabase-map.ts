...
export class SupabaseMap<K extends string, V extends Record<string, any>> implements Map<string, V> {
  ...
  static async create<K extends string, V extends Record<string, any>>(
    ...
    if (error) {
      ...
      // Fall back to in-memory with initial data
    ...
    if (data.length === 0 && initialData.length > 0) {
      for (const item of initialData) {
        const { error: insertErr } = await client.from(table).insert(item)
        if (!insertErr) map.set(String(item[pkField]), item)
      }
    } else {
      data.forEach((row: V) => map.set(String(row[pkField]), row))
    }
  ...
  set(key: string, value: V): this {
    ...
    const client = getClient()
    client.from(this.table).upsert(value).then(({ error }) => {
  ...
  delete(key: string): boolean {
    ...
    if (existed) {
      const client = getClient()
      client.from(this.table).delete().eq(this.pkField, key).then(({ error }) => {
  ...
  clear(): void {
    this.cache.clear()
    const client = getClient()
    client.from(this.table).delete().neq(this.pkField, '').then(({ error }) => {
