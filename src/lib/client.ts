...
function tok() {
  return typeof window !== 'undefined' ? localStorage.getItem('fib_token') || '' : ''
}
...
export const getOperativosPublicos = (p?: Record<string,string>) =>
  fetch('/api/operativos?publica=1'+(p?'&'+new URLSearchParams(p):'')).then(r=>r.json())
...
export const crearDM       = (participantes:string[]) => api<any>('/chat',{method:'POST',body:JSON.stringify({tipo:'dm',participantes})})
