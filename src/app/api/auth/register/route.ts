...
export async function POST(req: NextRequest) {
  ...
  const inv = DB.invites.get(codigo.trim())
  ...
  for (const u of DB.users.values())
    if (u.username.toLowerCase() === username.toLowerCase()) return err('Ese nombre de usuario ya existe')
  const id = uuid()
  const passwordHash = await bcrypt.hash(password, 12)
  const user = { id, username:username.trim(), passwordHash, rol:inv.rol, discordId:inv.discordId, agentNumber:inv.agentNumber, nombre:nombre?.trim()||inv.nombre||null, callsign:null, createdAt:new Date().toISOString(), activo:true }
  DB.users.set(id, user)
  inv.usos++; inv.usadoPor.push(username.trim())
  logRegister(user.username, user.rol, codigo.trim())
